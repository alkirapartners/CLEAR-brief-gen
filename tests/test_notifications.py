...
def test_format_message_includes_email_company_score():
    msg = _format_message("user@example.com", "Mary Kay", 82)
    assert "user@example.com" in msg
    assert "Mary Kay" in msg
    assert "82" in msg


def test_format_message_green_at_80():
    msg = _format_message("u@x.com", "Acme", 80)
    assert ":large_green_circle:" in msg


def test_format_message_green_above_80():
    msg = _format_message("u@x.com", "Acme", 99)
    assert ":large_green_circle:" in msg


def test_format_message_yellow_at_60():
    msg = _format_message("u@x.com", "Acme", 60)
    assert ":large_yellow_circle:" in msg


def test_format_message_yellow_at_79():
    msg = _format_message("u@x.com", "Acme", 79)
    assert ":large_yellow_circle:" in msg


def test_format_message_red_below_60():
    msg = _format_message("u@x.com", "Acme", 59)
...
def test_format_message_includes_utc_timestamp():
    msg = _format_message("u@x.com", "Acme", 75)
    assert "UTC" in msg
...
def test_slack_webhook_url_returns_empty_when_unset(monkeypatch):
    ...
    # st.secrets will raise FileNotFoundError in test env — function must swallow it.
...
def test_notify_returns_false_when_webhook_unset(monkeypatch):
    ...
    with patch("notifications.st") as mock_st:
        ...
        result = notify_brief_generated("u@x.com", "Acme", 75)
...
def test_notify_returns_true_on_successful_post(monkeypatch):
    ...
    with patch("notifications._post") as mock_post:
        ...
        result = notify_brief_generated("u@x.com", "Acme", 82)
    ...
    assert "82" in posted_payload["text"]
...
def test_notify_returns_false_on_post_failure(monkeypatch):
    ...
    with patch("notifications._post", side_effect=URLError("network down")):
        result = notify_brief_generated("u@x.com", "Acme", 70)
    assert result is False
